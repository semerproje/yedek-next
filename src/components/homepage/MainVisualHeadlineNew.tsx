...
export default memo(function MainVisualHeadline({ 
        ...
        try {
          ...
          if (manualNewsIds.length > 0) {
            const manualQuery = query(
              collection(db, 'news'),
              where(documentId(), 'in', manualNewsIds.slice(0, 10)), // Firestore'da 'in' sorgusu max 10 element alır
          ...
          if (autoFetch && newsData.length < newsCount) {
            ...
            // En son haberleri getir (manuel seçilmişleri hariç)
            const autoQuery = query(
              collection(db, 'news'),
              where('status', '==', 'published'),
              orderBy('publishedAt', 'desc'),
              limit(neededCount + existingIds.length) // Biraz fazla çek filtreleme için
            );
            ...
            const autoNews = snapshot.docs
              ...
              .filter(n => !existingIds.includes(n.id)) // Manuel seçilmişleri hariç tut
              .slice(0, neededCount);
          ...
          if (newsData.length > 0) {
            setNews(newsData);
            console.log('✅ Main visual headline news loaded:', newsData.length);
          } else {
  ...
  const { displayNews, currentNews, leftItems, rightItems } = useMemo(() => {
    ...
    const leftItems = displayNews.slice(1, Math.min(1 + settings.leftSideCount, displayNews.length));
    const rightItems = displayNews.slice(1 + settings.leftSideCount, Math.min(1 + settings.leftSideCount + settings.rightSideCount, displayNews.length));
      ...
      <div className="w-full max-w-[1530px] flex flex-row items-stretch gap-0">
        ...
        <aside className="hidden xl:flex flex-col w-[320px] min-w-[290px] max-w-[340px] border-r border-gray-100 pr-7">
          <ScrollList
            ...
            activeIndex={activeIndex >= 1 && activeIndex <= settings.leftSideCount ? activeIndex - 1 : -1}
          />
        </aside>
        ...
        <aside className="hidden xl:flex flex-col w-[320px] min-w-[290px] max-w-[340px] border-l border-gray-100 pl-7">
          <ScrollList
            ...
            onItemClick={(_, idx) => setActiveIndex(idx + 1 + settings.leftSideCount)} // Ana + sol haberler sonrası
            activeIndex={activeIndex >= 1 + settings.leftSideCount && activeIndex < 1 + settings.leftSideCount + settings.rightSideCount ? activeIndex - 1 - settings.leftSideCount : -1}
          />
        </aside>
      </div>
