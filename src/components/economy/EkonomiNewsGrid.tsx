...
export default function EkonomiNewsGrid() {
  ...
  if (!newsList.length)
    return (
      <div className="text-center py-10 text-lg text-gray-600 dark:text-gray-300">
        Ekonomi kategorisinde haber bulunamadı.
      </div>
    );

  return (
    <section className="container mx-auto px-2">
      <h2 className="text-2xl font-bold mb-6 text-gray-900 dark:text-white">Ekonomi Haberleri</h2>
        ...
        {newsList.map((item) => (
          ...
          >
            <Link href={`/haber/${item.id}`}>
              ...
              <div className="inline-block mt-3 px-2 py-1 bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-300 rounded text-xs font-semibold">
                Ekonomi
              </div>
            </Link>
          </article>
