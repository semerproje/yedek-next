...
export default function DinNewsGrid() {
  ...
  if (!newsList.length)
    return (
      <div className="text-center py-10 text-lg text-gray-600 dark:text-gray-300">
        Din kategorisinde haber bulunamadı.
      </div>
    );

  return (
    <section className="container mx-auto px-2">
      <h2 className="text-2xl font-bold mb-6 text-gray-900 dark:text-white">Din Haberleri</h2>
        ...
        {newsList.map((item) => (
          ...
          >
            <Link href={`/haber/${item.id}`}>
              ...
              <div className="inline-block mt-3 px-2 py-1 bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 rounded text-xs font-semibold">
                Din
              </div>
            </Link>
          </article>
