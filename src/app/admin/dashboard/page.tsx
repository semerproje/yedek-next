...
export default function Dashboard() {
  ...
  const loadDashboardData = async () => {
    ...
    if (process.env.NODE_ENV === 'development' && process.env.NEXT_PUBLIC_FIREBASE_DEV_MODE === 'true') {
      console.log('🚧 Development mode: Using demo data')
      setStats({
        totalNews: 125,
        publishedNews: 98,
        draftNews: 27,
      ...
      setRecentNews([
        {
          id: 'demo1',
          title: 'Demo Haber 1 - Teknoloji Gelişmeleri',
          category: 'teknoloji',
        ...
        {
          id: 'demo2', 
          title: 'Demo Haber 2 - Ekonomi Haberleri',
          category: 'ekonomi',
          publishedAt: new Date().toISOString(),
          createdAt: new Date(),
          views: 890
    ...
    try {
      ...
      // Optimize Firebase queries with limits and indexing
      const [
        newsSnapshot,
        publishedSnapshot,
        draftSnapshot,
        ...
        getDocs(query(collection(db, 'news'), limit(1000))), // Add limit
        getDocs(query(collection(db, 'news'), where('status', '==', 'published'), limit(500))),
        getDocs(query(collection(db, 'news'), where('status', '==', 'draft'), limit(200))),
        getDocs(query(collection(db, 'categories'), limit(50))),
        getDocs(query(collection(db, 'aa_news'), limit(100))),
        getDocs(query(collection(db, 'news'), where('featured', '==', true), limit(20))),
        getDocs(query(
          collection(db, 'news'), 
                  ...
                  <div key={news.id} className="flex items-start space-x-3 p-3 rounded-lg hover:bg-gray-50 transition-colors">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-500 mt-1">
                        {news.createdAt ? new Date(news.createdAt).toLocaleDateString('tr-TR') : 
                         news.publishedAt ? new Date(news.publishedAt).toLocaleDateString('tr-TR') : 
                         'Tarih bilinmiyor'}
                      </p>
                    </div>
                  </div>
